...
public class Deobfuscator {
    ...
    public void execute() {
        if (args.getDeobfuscationMapFileMode().shouldRead()) {
            if (deobfPresets.load()) {
                ...
                // not needed anymore
                deobfPresets.getPkgPresetMap().clear();
                initIndexes();
            }
        }
        process();
    ...
    private void preProcessClass(ClassNode cls) {
        ...
        if (alias != null) {
            ...
            if (!clsMap.containsKey(classInfo)) {
                ...
                boolean badName = shouldRename(clsShortName) || (args.isRenameValid() && reservedClsNames.contains(clsShortName));
    ...
    private void processPackageFull(PackageNode pkg, String fullName) {
        ...
        if (!pkg.hasAlias()) {
            ...
            if (// check if first level is a valid tld
            (args.isDeobfuscationOn() && shouldRename(pkgName)) && (pkg.getParentPackage() != rootPackage || !TldHelper.contains(pkgName)) || (args.isRenameValid() && !NameMapper.isValidIdentifier(pkgName)) || (args.isRenamePrintable() && !NameMapper.isAllCharsPrintable(pkgName))) {
