...
public class MarkFinallyVisitor extends AbstractVisitor {
    ...
    private static void cutPathEnds(MethodNode mth, List<BlockNode> handlerBlocks) {
        List<BlockNode> throwBlocks = ListUtils.filter(handlerBlocks, b -> BlockUtils.checkLastInsnType(b, InsnType.THROW));
    ...
    private static boolean checkSlices(FinallyExtractInfo extractInfo) {
        ...
        for (InsnsSlice dupSlice : extractInfo.getDuplicateSlices()) {
            ...
            if (dupInsnsList.size() != finallyInsnsList.size()) {
                extractInfo.getMth().addDebugComment("Incorrect finally slice size: " + dupSlice + ", expected: " + finallySlice);
                return false;
            }
        }
        for (int i = 0; i < finallyInsnsList.size(); i++) {
            ...
            for (InsnsSlice dupSlice : extractInfo.getDuplicateSlices()) {
                ...
                if (finallyInsn.getType() != dupInsn.getType()) {
                    extractInfo.getMth().addDebugComment("Incorrect finally slice insn: " + dupInsn + ", expected: " + finallyInsn);
                    return false;
                }
            }
        }
    ...
    @Nullable
    private static InsnsSlice searchFromFirstBlock(BlockNode dupBlock, @Nullable BlockNode startBlock, FinallyExtractInfo extractInfo) {
        InsnsSlice dupSlice = isStartBlock(dupBlock, startBlock, extractInfo);
        if (dupSlice == null) {
            return null;
        }
        if (!dupSlice.isComplete() && !checkBlocksTree(dupBlock, startBlock, dupSlice, extractInfo)) {
            return null;
        }
    ...
    private static InsnsSlice isStartBlock(BlockNode dupBlock, @Nullable BlockNode finallyBlock, FinallyExtractInfo extractInfo) {
        ...
        if (finallySlice.isComplete()) {
            ...
            if (finallySlice.getInsnsList().size() != slice.getInsnsList().size()) {
                extractInfo.getMth().addDebugComment("Another duplicated slice has different insns count: " + slice + ", finally: " + finallySlice);
                return null;
            }
    ...
    private static boolean checkBlocksTree(BlockNode dupBlock, @Nullable BlockNode finallyBlock, InsnsSlice dupSlice, FinallyExtractInfo extractInfo) {
        InsnsSlice finallySlice = extractInfo.getFinallyInsnsSlice();
    ...
    private static boolean isSameArgs(FinallyExtractInfo extractInfo, InsnArg dupArg, InsnArg fArg) {
        ...
        if (isReg) {
            ...
            if (!dupReg.sameCodeVar(fReg) && !sameDebugInfo(dupReg, fReg) && assignedOutsideHandler(extractInfo, dupReg, fReg) && assignInsnDifferent(dupReg, fReg)) {
                return false;
            }
        }
