...
public class BlockProcessor extends AbstractVisitor {
    ...
    private static void updateExitBlocks(MethodNode mth) {
        ...
        mth.getBasicBlocks().forEach(block -> {
            ...
            } else if (noSuccessors && !mth.isVoidReturn() && !mth.isConstructor()) {
                mth.addComment("JADX INFO: Unexpected exit block: " + block + ". Expect last instruction to be RETURN or THROW, got: " + lastInsn);
            }
        });
    }
    ...
    private static boolean checkLoops(MethodNode mth, BlockNode block) {
        ...
        if (loopsCount == 1) {
            ...
            return insertBlocksForBreak(mth, loop) || insertBlocksForContinue(mth, loop) || insertBlockForProdecessors(mth, loop) || insertPreHeader(mth, loop);
        }
        return false;
    }
    ...
    private static boolean splitReturn(MethodNode mth, BlockNode exitBlock) {
        if (exitBlock.contains(AFlag.SYNTHETIC) || exitBlock.contains(AFlag.ORIG_RETURN) || exitBlock.contains(AType.SPLITTER_BLOCK)) {
            return false;
        }
        ...
        if (returnInsn.getArgsCount() == 1 && exitBlock.getInstructions().size() == 1 && !isReturnArgAssignInPred(preds, returnInsn)) {
            return false;
        }
        boolean first = true;
