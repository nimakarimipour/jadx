...
public class LoopRegionVisitor extends AbstractVisitor implements IRegionVisitor {
    ...
    private static boolean fixIterableType(MethodNode mth, InsnArg iterableArg, RegisterArg iterVar) {
        ...
        if (iterableType.isGeneric()) {
            ...
            if (wildcardType != null && gType.getWildcardBound() == ArgType.WildcardBound.EXTENDS && ArgType.isInstanceOf(mth.root(), wildcardType, varType)) {
                return true;
            }
     ...
     */
    private static boolean checkInvoke(InsnNode insn, @Nullable() String declClsFullName, String mthId) {
        if (insn == null) {
            return false;
        }
