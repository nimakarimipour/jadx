...
@JadxVisitor(name = "SSATransform", desc = "Calculate Single Side Assign (SSA) variables", runAfter = BlockProcessor.class)
public class SSATransform extends AbstractVisitor {
    ...
    private static void fixPhiInTryCatch(PhiInsn phi) {
        ...
        while (k < argsCount) {
            ...
            if (parentInsn != null && parentInsn.getResult() != null && parentInsn.contains(AFlag.TRY_LEAVE) && phi.removeArg(arg)) /* TODO: fix registers removing */
            {
                argsCount--;
                continue;
            }
