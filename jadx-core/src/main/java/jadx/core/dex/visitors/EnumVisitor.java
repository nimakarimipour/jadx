...
@JadxVisitor(name = "EnumVisitor", desc = "Restore enum classes", runAfter = { // all possible instructions already inlined
// ternary operator inlined
CodeShrinkVisitor.class, // ternary operator inlined
ModVisitor.class, // ternary operator inlined
ReSugarCode.class, // regions processing finished
IfRegionVisitor.class, CheckRegions.class }, runBefore = { ExtractFieldInit.class })
public class EnumVisitor extends AbstractVisitor {
    ...
    public void init(RootNode root) {
        enumValueOfMth = MethodInfo.fromDetails(root, ClassInfo.fromType(root, ArgType.ENUM), "valueOf", Arrays.asList(ArgType.CLASS, ArgType.STRING), ArgType.ENUM);
        cloneMth = MethodInfo.fromDetails(root, ClassInfo.fromType(root, ArgType.OBJECT), "clone", Collections.emptyList(), ArgType.OBJECT);
    }
    ...
    private boolean searchValuesField(EnumData data) {
        ...
        List<FieldNode> valuesCandidates = data.cls.getFields().stream().filter(f -> f.getAccessFlags().isStatic()).filter(f -> f.getType().isArray()).filter(f -> Objects.equals(f.getType().getArrayRootElement(), clsType)).collect(Collectors.toList());
    ...
    private boolean isDefaultConstructor(MethodNode mth, String shortId) {
        boolean defaultId = shortId.equals("<init>(Ljava/lang/String;I)V") || shortId.equals("<init>(Ljava/lang/String;)V");
        if (defaultId) {
            // check content
    ...
    private void fixValuesAccess(MethodNode mth, FieldInfo valuesFieldInfo, ArgType clsType, @Nullable MethodNode valuesMethod) {
        ...
        InsnUtils.replaceInsns(mth, insn -> {
            if (insn.getType() == InsnType.SGET && insnTest.test(insn)) {
                MethodInfo valueMth = valuesMethod == null ? getValueMthInfo(mth.root(), clsType) : valuesMethod.getMethodInfo();
    ...
    private MethodInfo getValueMthInfo(RootNode root, ArgType clsType) {
        return MethodInfo.fromDetails(root, ClassInfo.fromType(root, clsType), "values", Collections.emptyList(), ArgType.array(clsType));
    }
