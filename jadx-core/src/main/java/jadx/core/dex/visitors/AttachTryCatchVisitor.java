...
public class AttachTryCatchVisitor extends AbstractVisitor {
    ...
    private static void initTryCatches(MethodNode mth, @Nullable ICodeReader codeReader, @Nullable InsnNode[] insnByOffset) {
        ...
        if (handlersCount > 0 && handlersCount != addrs.size()) {
            ...
            for (TryCatchBlock outerTry : catches) {
                for (TryCatchBlock innerTry : catches) {
                    if (outerTry != innerTry && innerTry.containsAllHandlers(outerTry)) {
