...
public class AttachTryCatchVisitor extends AbstractVisitor {
    ...
    private static ArgType maxCatchFromHandler(ExceptionHandler handler, TypeCompare typeCompare) {
        ...
        return catchTypes.stream().map(ClassInfo::getType).max(typeCompare.getComparator()).orElseThrow(() -> new JadxRuntimeException("Failed to get max type from catch list: " + catchTypes));
    }
