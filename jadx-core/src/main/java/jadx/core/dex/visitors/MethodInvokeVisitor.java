...
@JadxVisitor(name = "MethodInvokeVisitor", desc = "Process additional info for method invocation (overload, vararg)", runAfter = { CodeShrinkVisitor.class, ModVisitor.class }, runBefore = { // run before cast remove and StringBuilder replace
...
public class MethodInvokeVisitor extends AbstractVisitor {
    ...
    @Nullable
    private ArgType getCallClassFromInvoke(MethodNode parentMth, BaseInvokeNode invokeInsn, @Nullable MethodInfo callMth) {
    ...
    private List<ArgType> searchCastTypes(MethodNode parentMth, IMethodDetails mthDetails, List<IMethodDetails> overloadedMethods, List<ArgType> compilerVarTypes) {
        ...
        if (Consts.DEBUG_OVERLOADED_CASTS) {
            ...
            parentMth.addComment("JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead" + NL + " method: " + mthDetails + NL + " arg types: " + compilerVarTypes + NL + " candidates:" + NL + "  " + Utils.listToString(overloadedMethods, NL + "  "));
        }
