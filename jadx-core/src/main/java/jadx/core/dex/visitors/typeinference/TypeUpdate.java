...
public final class TypeUpdate {
     ...
     */
    public TypeUpdateResult applyWithWiderAllow(MethodNode mth, SSAVar ssaVar, @Nullable ArgType candidateType) {
    ...
    private TypeUpdateResult apply(MethodNode mth, SSAVar ssaVar, @Nullable ArgType candidateType, TypeUpdateFlags flags) {
    ...
    private TypeUpdateResult moveListener(TypeUpdateInfo updateInfo, InsnNode insn, InsnArg arg, ArgType candidateType) {
        ...
        if (result == SAME && !correctType) {
            if (Consts.DEBUG_TYPE_INFERENCE) {
                LOG.debug("Move insn types mismatch: {} -> {}, change arg: {}, insn: {}", candidateType, changeArg.getType(), changeArg, insn);
            }
            return REJECT;
        }
