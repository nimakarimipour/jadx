...
public class DotGraphVisitor extends AbstractVisitor {
    ...
    private class DumpDotGraph {
        ...
        public void process(MethodNode mth) {
            ...
            dot.add(escape(mth.getReturnType() + " " + mth.getParentClass() + '.' + mth.getName() + '(' + Utils.listToString(mth.getAllArgRegs()) + ") "));
            ...
            File file = dir.toPath().resolve(mth.getParentClass().getClassInfo().getAliasFullPath() + "_graphs").resolve(fileName).toFile();
        ...
        private String escape(String string) {
            return string.replace("\\", // TODO replace \"
            "").replace("/", "\\/").replace(">", "\\>").replace("<", "\\<").replace("{", "\\{").replace("}", "\\}").replace("\"", "\\\"").replace("-", "\\-").replace("|", "\\|").replace(CodeWriter.NL, NL).replace("\n", NL);
        }
    }
}
