...
public class InsnDecoder {
	...
	private InsnNode makeNewArray(InsnData insn) {
		...
		if (dim == 0) {
			...
			if (Nullability.castToNonnull(indexType, "exception if invalid").isArray()) {
				arrType = indexType;
			} else {
	...
	private InsnNode filledNewArray(InsnData insn, boolean isRange) {
		...
		ArgType elType = Nullability.castToNonnull(arrType, "framework ensures non-null").getArrayElement();
