...
public class InsnRemover {
    ...
    private static void removeSsaVar(MethodNode mth, SSAVar ssaVar) {
        ...
        if (Consts.DEBUG_WITH_ERRORS) {
            throw new JadxRuntimeException("Can't remove SSA var, still in use, count: " + useCount + ", list:" + NL + "  " + ssaVar.getUseList().stream().map(arg -> arg + " from " + arg.getParentInsn()).collect(Collectors.joining(NL + "  ")));
        }
    }
    ...
    private static void removeAll(List<InsnNode> insns, List<InsnNode> toRemove) {
        ...
        for (InsnNode rem : toRemove) {
            ...
            if (!found && Consts.DEBUG_WITH_ERRORS) {
                throw new JadxRuntimeException("Can't remove insn:" + NL + "  " + rem + NL + " not found in list:" + NL + "  " + Utils.listToString(insns, NL + "  "));
            }
        }
    }
