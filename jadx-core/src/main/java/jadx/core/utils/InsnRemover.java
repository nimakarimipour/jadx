...
public class InsnRemover {
    ...
    public static void unbindResult(@Nullable MethodNode mth, InsnNode insn) {
        ...
        // don't unset result arg, can be used to restore variable
        r.add(AFlag.REMOVE);
        if (mth == null) {
            return;
        }
    ...
    private static void removeAll(List<InsnNode> insns, List<InsnNode> toRemove) {
        ...
        for (InsnNode rem : toRemove) {
            ...
            if (!found && Consts.DEBUG_WITH_ERRORS) {
                throw new JadxRuntimeException("Can't remove insn:" + ICodeWriter.NL + "  " + rem + ICodeWriter.NL + " not found in list:" + ICodeWriter.NL + "  " + Utils.listToString(insns, ICodeWriter.NL + "  "));
            }
        }
    }
